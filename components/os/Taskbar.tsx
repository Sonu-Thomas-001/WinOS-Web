...
import { Wifi, Volume2, Battery, ChevronUp, Search, Power } from 'lucide-react';
import StartMenu from './StartMenu';

const Taskbar: React.FC = () => {
  const { windows, activeWindowId, launchApp, focusWindow, minimizeWindow, toggleStartMenu, system, toggleCalendar } = useOS();
  ...
  const pinnedApps = [AppID.EXPLORER, AppID.BROWSER, AppID.NOTEPAD, AppID.TERMINAL, AppID.SETTINGS];
  
  // Get unique list of relevant app IDs (pinned + open)
  ...
  return (
    <>
      ...
      {/* Calendar / System Tray Popup (Simulated) */}
      {system.isCalendarOpen && (
        <div className="absolute bottom-12 right-2 w-80 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md p-4 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 z-[9999] animate-slide-up">
           <h2 className="text-xl font-semibold mb-2 text-gray-800 dark:text-white">{formatDate(time)}</h2>
      ...
      <div className="fixed bottom-0 w-full h-12 bg-white/80 dark:bg-[#202020]/85 backdrop-blur-xl border-t border-white/20 dark:border-white/5 flex justify-between items-center px-2 z-[9999] select-none">
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="hidden sm:flex items-center bg-gray-100 dark:bg-white/5 rounded-full px-3 py-1 ml-2 border border-gray-300 dark:border-gray-600/50">
          ...
          {taskbarAppIds.map(appId => {
            ...
            return (
              ...
              >
                ...
                {/* Tooltip */}
                <div className="absolute bottom-12 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-800 text-white text-xs py-1 px-2 rounded pointer-events-none whitespace-nowrap">
                   {config.title}
                </div>
              </button>
            );
        ...
        <div className="flex items-center gap-2 h-full">
           ...
           <div className="flex items-center gap-2 hover:bg-white/50 dark:hover:bg-white/10 p-1 rounded px-2 cursor-pointer transition-colors">
           ...
           <div 
             className="flex flex-col items-end justify-center px-2 hover:bg-white/50 dark:hover:bg-white/10 h-full rounded cursor-pointer transition-colors"
